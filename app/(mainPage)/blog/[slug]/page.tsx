  ...
  return !event ? (
    ...
    <Fragment>
      <Image
        ...
        height={1000}
        alt={event?.title}
        className="w-full rounded-br-[6rem] lg:rounded-br-[13rem]"
      />
      <div className="flex flex-col gap-2">
        ...
        <ul className="flex gap-4 my-2">
          {event.tags?.map((tag: any) => (
            <li
              key={tag._id}
              className={`px-2 py-1 rounded-xl text-md font-bold`}
      ...
      <div className={classes.post}>
        <PortableText value={event?.body} components={component} />
      </div>
    </Fragment>
  );
};
...
export async function generateStaticParams() {
  const events: any = await getPosts();
