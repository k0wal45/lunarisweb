...
const BlogItem = (post: any) => {
  return (
    ...
    >
      <Image
        src={urlForImage(post.post.mainImage).url()}
        alt={post.post.title}
        width={400}
        height={300}
        className="w-full h-48 rounded-xl group-hover:translate-y-[-4px] group-hover:translate-x-2
        duration-200 transition-all"
      />
      <div
      ...
      >
        ...
        <span className="text-sm text-base">
          {new Date(post?.post.publishedAt).toDateString()}
        </span>
      </div>
    </TransitionLink>
  );
